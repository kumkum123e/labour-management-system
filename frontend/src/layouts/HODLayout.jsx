...
import { FiGrid, FiClock, FiCheck, FiX, FiList, FiLogOut, FiMenu } from "react-icons/fi";
import { useAuth } from "../hooks/useAuth";

const links = [
  { to: "/hod/dashboard", label: "Dashboard", icon: FiGrid, end: true },
  { to: "/hod/pending", label: "Pending Requests", icon: FiClock },
  { to: "/hod/approved", label: "Approved", icon: FiCheck },
  { to: "/hod/rejected", label: "Rejected", icon: FiX },
  { to: "/hod/history", label: "History", icon: FiList },
];
...
export default function HODLayout() {
  ...
  if (!user || (!hasRole("HOD") && !hasRole("ADMIN"))) return <Navigate to="/hod-login" replace />;
    ...
    <div className="flex min-h-screen bg-slate-100 relative">
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-700 p-5">
          <div>
            <h2 className="text-lg font-semibold text-white">HOD Panel</h2>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-6 md:hidden">
          <h2 className="text-lg font-bold text-slate-900">HOD Panel</h2>
          <button 
            type="button" 
